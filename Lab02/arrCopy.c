...
void printArr(int *a, int size, char *prompt){
	// Your code here
    printf("%s", prompt);
    if (size == 0) { // print empty if no elements in array
        printf("empty");
    }
    else { // else print array
...
void arrCopy(){
	// Your code here
    for (int i = 0; i < size; i++) { // Count the number of even and odd numbers and fill the arrays
        if (*(arr + i) % 2 == 0) {
            evenCount++;
        } else {
            oddCount++;
    ...
    for (int i = 0; i < size; i++) {
        if (*(arr + i) % 2 == 0) {
            *(arr_even + evenIndex++) = *(arr + i); // copy even elements into arr_even
        }
        if (*(arr + i) % 2 != 0) {
            *(arr_odd + oddIndex++) = *(arr + i); // copy odd elements into arr_odd
        }
    }
}
